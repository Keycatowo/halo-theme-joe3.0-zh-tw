function getChildren(t,e){for(var s of t.children)if(s.className===e)return s;return null}document.addEventListener("DOMContentLoaded",()=>{$(".joe_detail__article p:empty").remove(),customElements.define("joe-mtitle",class extends HTMLElement{constructor(){super(),this.innerHTML=`
			...
			`}}),customElements.define("joe-mp3",class extends HTMLElement{constructor(){super(),this.options={name:this.getAttribute("name"),url:this.getAttribute("url"),theme:this.getAttribute("theme")||"#1989fa",cover:this.getAttribute("cover")||"",autoplay:!!this.getAttribute("autoplay")},this.render()}render(){if(!this.options.url)return this.innerHTML="音訊地址未填寫！";this.innerHTML='<span style="display: block" class="_content"></span>',new APlayer({container:getChildren(this,"_content"),theme:this.options.theme,autoplay:this.options.autoplay,audio:[{url:this.options.url,name:this.options.name,cover:this.options.cover}]})}}),customElements.define("joe-music",class extends HTMLElement{constructor(){super(),this.options={id:this.getAttribute("id"),color:this.getAttribute("color")||"#1989fa",autoplay:!!this.getAttribute("autoplay")},this.render()}render(){if(!this.options.id)return this.innerHTML="網易雲歌曲ID未填寫！";this.innerHTML='<span style="display: block" class="_content"></span>',fetch("https://www.vvhan.com/usr/themes/Joe/NeteaseCloudMusicApi.php?id="+this.options.id).then(async t=>{t=await t.json();new APlayer({container:getChildren(this,"_content"),lrcType:1,theme:this.options.color,autoplay:this.options.autoplay,audio:t})})}}),customElements.define("joe-mlist",class extends HTMLElement{constructor(){super(),this.options={id:this.getAttribute("id"),color:this.getAttribute("color")||"#1989fa",autoplay:!!this.getAttribute("autoplay")},this.render()}render(){if(!this.options.id)return this.innerHTML="網易雲歌單ID未填寫！";this.innerHTML='<span style="display: block" class="_content"></span>',fetch("https://api.i-meto.com/meting/api?server=netease&type=playlist&id="+this.options.id).then(async t=>{t=await t.json();new APlayer({container:getChildren(this,"_content"),lrcType:3,theme:this.options.color,autoplay:this.options.autoplay,audio:t})})}}),customElements.define("joe-abtn",class extends HTMLElement{constructor(){super(),this.options={icon:this.getAttribute("icon")||"",color:this.getAttribute("color")||"#ff6800",href:this.getAttribute("href")||"#",radius:this.getAttribute("radius")||"17.5px",content:this.getAttribute("content")||"多彩按鈕"},this.innerHTML=`
				...
				`}}),customElements.define("joe-cloud",class extends HTMLElement{constructor(){super(),this.options={type:this.getAttribute("type")||"default",title:this.getAttribute("title")||"預設標題",url:this.getAttribute("url"),password:this.getAttribute("password")};this.innerHTML=`
					<span class="joe_cloud">
						<div class="joe_cloud__logo _${this.options.type}"></div>
						<div class="joe_cloud__describe">
							<div class="joe_cloud__describe-title">${this.options.title}</div>
							<div class="joe_cloud__describe-type">來源：${{default:"預設網盤",360:"360網盤",bd:"百度網盤",ty:"天翼網盤",ct:"城通網盤",wy:"微雲網盤",github:"Github倉庫",gitee:"Gitee倉庫",lz:"藍奏雲網盤",ad:"阿里雲盤"}[this.options.type]||"預設網盤"}${this.options.password?" | 提取碼："+this.options.password:""}</div>
						</div>
						<a class="joe_cloud__btn" href="${this.options.url}" target="_blank" rel="noopener noreferrer nofollow">
							<i class="fa fa-download"></i>
						</a>
					</span>
				`}}),customElements.define("joe-read-limited",class extends HTMLElement{constructor(){super(),this.render()}render(){this.innerHTML='<div class="joe_read_limited"><p><i class="joe-font joe-icon-locker" style="color:#f5840d;"></i>&nbsp;此處內容僅 <span class="joe_read_limited__button">評論</span> 後可見</p></div>',this.$button=this.querySelector(".joe_read_limited__button");const e=document.querySelector(".joe_comment"),s=document.querySelector(".joe_header");var t=$(".joe_detail").attr("data-cid");console.log(t),e&&s&&this.$button.addEventListener("click",t=>{t.stopPropagation(),Boolean(document.querySelector('[id*="comment-"]'))||Boolean(document.querySelector("#waline"))?(t=e.offsetTop-s.offsetHeight-15,$("html,body").animate({scrollTop:t},500)):Qmsg.warning("評論功能不可用！")})}}),customElements.define("joe-card-default",class extends HTMLElement{constructor(){super();var t,e=getChildren(this,"_temp"),e=(this.options={width:this.getAttribute("width")||"100%",label:this.getAttribute("label")||"卡片標題",content:e.innerHTML.trim().replace(/^(<br>)|(<br>)$/g,"")||"卡片內容"},`
			...
			`}}),customElements.define("joe-callout",class extends HTMLElement{constructor(){super();var t,e=getChildren(this,"_temp"),e=(this.options={color:this.getAttribute("color")||"#f0ad4e",content:e.innerHTML.trim().replace(/^(<br>)|(<br>)$/g,"")||"標註內容"},`
					<div class="joe_callout" style="border-left-color: ${this.options.color};">
						${this.options.content}
					</div>
				`);getChildren(this,"_content")?getChildren(this,"_content").innerHTML=e:((t=document.createElement("span")).style.display="block",t.className="_content",t.innerHTML=e,this.appendChild(t))}}),customElements.define("joe-card-describe",class extends HTMLElement{constructor(){super();var t,e=getChildren(this,"_temp"),e=(this.options={title:this.getAttribute("title")||"卡片描述",content:e.innerHTML.trim().replace(/^(<br>)|(<br>)$/g,"")||"卡片內容"},`
